...
const SOURCES: Record<string, GrammarSource> = {
  ...
  // Ascend C reuses tree-sitter-cpp — the preprocessor strips non-standard
  // keywords before parsing; see LanguageProvider.preprocessSource.
  [SupportedLanguages.AscendC]: {
    load: () => _require('tree-sitter-cpp'),
    unavailableNote:
      'Ascend C parsing requires `tree-sitter-cpp` (shared grammar). ' +
      'Check the install and native binding.',
  },
};
